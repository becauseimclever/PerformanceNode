...
#   2. Clones the Pico SDK (including submodules) to /opt/runner-cache/pico-sdk at
#      the pinned PICO_SDK_VERSION tag.  Skips clone if already at that tag.
#   3. Creates /opt/runner-cache/ccache for ccache's object cache.
#   4. Writes a runner systemd drop-in so the runner process sees PICO_SDK_PATH
...
#   PICO_SDK_DIR        Host path for the SDK            (default: /opt/runner-cache/pico-sdk)
#   CCACHE_CACHE_DIR    Host path for ccache objects     (default: /opt/runner-cache/ccache)
...
PICO_SDK_DIR="${PICO_SDK_DIR:-/opt/runner-cache/pico-sdk}"
CCACHE_CACHE_DIR="${CCACHE_CACHE_DIR:-/opt/runner-cache/ccache}"
...
chmod 1777 "$CCACHE_CACHE_DIR"

# ── 4. Systemd drop-in: expose PICO_SDK_PATH / CCACHE_DIR to runner ────────
DROP_IN="$(runner_detect_drop_in_dir)/10-pico-cache.conf"
...
runner_write_drop_in "10-pico-cache.conf" "$DROP_IN_CONTENT"
