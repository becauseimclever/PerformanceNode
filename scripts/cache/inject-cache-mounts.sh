...
#   /opt/cache/nuget     → /root/.nuget/packages   (rw)  — NuGet package cache
#   /opt/cache/pico-sdk  → /opt/pico-sdk            (ro)  — Pico SDK (read-only)
#   /opt/cache/ccache    → /root/.ccache            (rw)  — ccache objects
#
...
#   • actions-runner systemd service exists
#
...
DROP_IN="${SYSTEMD_DROP_IN_DIR}/20-cache-hooks.conf"
...
const CACHE_MOUNTS = [
  ['/opt/cache/nuget',    '/root/.nuget/packages', false],
  ['/opt/cache/pico-sdk', '/opt/pico-sdk',         true],
  ['/opt/cache/ccache',   '/root/.ccache',         false],
];
...
EOF
  ...
  echo "  $DROP_IN already exists — skipping (delete to regenerate)"
fi
...
missing=false
for dir in /opt/cache/nuget /opt/cache/pico-sdk /opt/cache/ccache; do
  if [ -d "$dir" ]; then
    echo "    OK   $dir"
  else
